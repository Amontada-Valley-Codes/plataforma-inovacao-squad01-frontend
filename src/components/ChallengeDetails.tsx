...
// 💡 ATUALIZANDO TIPO PARA INCLUIR STATUS
type ConnectionStatus = 'nenhum' | 'INTERESSE' | 'CONVIDADA' | 'POC' | 'REJEITADA';
...
export function ChallengeDetails({ user, challenge, onNavigate }: ChallengeDetailsProps) {
    ...
    const fetchRelatedData = async () => {
        ...
        try {
            const [ideasRes, startupsRes, commentsRes, connectionsRes] = await Promise.all([
                api.get('/idea'),
                api.get('/startups'),
            ...
            const recommendations = startupsRes.data.slice(0, 3).map((startup: Startup, index: number) => {
                ...
                return {
                    ...
                    connectionStatus: existingConnection ? existingConnection.status : 'nenhum',
                };
            });
    ...
    const handleConnect = async (startupId: string) => {
        ...
        try {
            ...
            // Ajuste os nomes dos campos para corresponder ao DTO do backend
            await api.post('/connections', {
                companyID: user.companyId, // de companyId para companyID
                startupID: startupId,      // de startupId para startupID
                challengeID: challenge.id,   // de challengeId para challengeID
                status: 'INTERESSE', 
                history: [] // Inicializa o histórico como um array vazio
            });
    ...
    // 💡 ATUALIZANDO FUNÇÕES DE STATUS PARA USAR OS NOVOS VALORES
        ...
        <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
            ...
            <div className="mt-8">
                <Tabs defaultValue="submissions">
                    ...
                    {/* Tab de Startups com botões funcionais */}
