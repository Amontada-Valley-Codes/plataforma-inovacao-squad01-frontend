...
import { User as UserType, Challenge, Idea as BaseIdea } from '../app/context/UserContext';
                    ...
                    <div className="flex gap-6 min-w-max h-full">
                        {funnelStages.map(stage => (
                            <Droppable key={stage.id} droppableId={stage.id}>
                                {(provided) => (
                                    <div ref={provided.innerRef} {...provided.droppableProps} className={`w-80 rounded-lg flex flex-col shadow-md border ${theme === "dark" ? "bg-gray-900 border-gray-700" : "bg-white border-gray-200"}`}>
                                                        ...
                                                        <Dialog open={isEvaluationModalOpen && selectedIdea?.id === idea.id} onOpenChange={(isOpen) => { if (!isOpen) setSelectedIdea(null); setIsEvaluationModalOpen(isOpen); }}>
                                                            ...
                                                            {isEvaluationStage(idea.stage) && (
                                                                <DialogContent className="bg-white">
                                                                    {selectedIdea && (
                                                                        <EvaluationForm idea={selectedIdea} user={user} onEvaluationComplete={() => { setIsEvaluationModalOpen(false); setSelectedIdea(null); fetchIdeas(); }} />
                                                                    )}
                                                                </DialogContent>
                                                            )}
                                                        </Dialog>
                                        ...
                                        <div className={`p-4 mt-auto ${theme === "dark" ? "bg-gray-900" : "bg-white"}`}>
                                            <Dialog open={isIdeaFormOpen} onOpenChange={setIsIdeaFormOpen}>
                                                <DialogTrigger asChild>
                                                    <Button variant="ghost" className={`w-full cursor-pointer ${theme === "dark" ? "text-white hover:bg-gray-700" : "text-[#001f61] hover:bg-gray-200"}`}>
                                                        <Plus className="w-4 h-4 mr-2" />
                                                        Adicionar Ideia
                                                    </Button>
                                                </DialogTrigger>
                                                <DialogContent className="bg-white">
                                                    <IdeaForm stageTitle={stage.title} onSubmit={handleIdeaSubmit} />
                                                </DialogContent>
                                            </Dialog>
                                        </div>
                                    </div>
