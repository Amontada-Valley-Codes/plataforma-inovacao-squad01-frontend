...
export function CompanyForm({ user, onNavigate }: CompanyFormProps) {
  ...
  const [description, setDescription] = useState(''); // Estado para o campo, embora não seja enviado
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !cnpj) {
        setError('Nome da empresa and CNPJ são obrigatórios.');
        setIsSubmitting(false);
        return;
    }

    try {
        // Envia apenas os campos que o DTO do backend espera
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-50 text-black'}`}>
      <div className={`bg-[#011677] text-white shadow-lg ${theme === 'dark' ? 'bg-gray-800' : 'bg-[#011677]'}`}>
      ...
      <div className={`container mx-auto px-6 py-8 max-w-2xl`}>
        {/* 4. Conectar o formulário ao handler */}
